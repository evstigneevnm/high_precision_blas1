...
template<class LinearOperator, class Preconditioner,
     class VectorOperations, class Monitor, class Log, unsigned int GMRESMaxBasisSize = 100>
...
private:
    ...
    typedef vectors_arr_wrap_static<VectorOperations,
                                    GMRESMaxBasisSize + 1>          bufs_arr_t;
    ...
    bool                 reorthogonalization_ = false;
    int                  restart_;
    ...
    void plane_rotation_(T_vec& H, T_vec& cs_, T_vec& sn_, T_vec& s, const int i) const
    {
        ...
        H[ (i+1)*restart_ + i] = T(0.0); //remove numerical noise below diagonal
...
public:
    ...
    void    set_reorthogonalization(bool flag) { reorthogonalization_ = flag; }
